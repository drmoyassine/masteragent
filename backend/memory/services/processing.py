"""memory/services/processing.py — Text chunking, summarization, PII scrubbing, document parsing, entity extraction"""
import base64
import json
import logging
import os
...
async def scrub_pii(text: str) -> str:
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.post(f"{config['api_base_url']}/redact", headers=headers, json={"text": text})
...
_EMPTY_EXTRACTION = {"entities": [], "intents": [], "relationships": []}
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                ...
                for e in response.json().get("entities", []):
                    label = e.get("label", "")
                    entity_type = "contact" if label == "person" else ("institution" if label == "organization" else "program")
                    entities.append({
