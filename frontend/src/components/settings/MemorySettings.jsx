...
function KnowledgeOperations({ runs, controls, eligibleCounts, eligibleCountsLoading, eligibleCountsError, onRefreshEligible, onRefresh, settings }) {
    ...
    const [batchPricing, setBatchPricing] = useState({ batch_input_cost_per_million: "", batch_output_cost_per_million: "", embedding_cost_per_million: "" });
    ...
    useEffect(() => {
        ...
        setBatchPricing({
            batch_input_cost_per_million: target.pricing?.batch_input_cost_per_million ?? "",
            batch_output_cost_per_million: target.pricing?.batch_output_cost_per_million ?? "",
            embedding_cost_per_million: target.pricing?.embedding_cost_per_million ?? "",
        });
    ...
    const operationOptions = () => ({ records_per_batch: recordsPerBatch, batches_per_run: runExtent === "all" ? undefined : resolvedBatches, run_all: runExtent === "all", max_records: totalRecords, max_clusters: maxClusters, mode: operation === "hygiene_analysis" ? (settings?.knowledge_hygiene_mode || "analysis_only") : undefined, provider_config_id: providerConfigId || undefined, model_name: batchModel || undefined, pricing: batchPricing });
