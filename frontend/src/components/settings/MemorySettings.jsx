...
const OPERATION_DEFINITIONS = {
    embedding_backfill: { label: "Embedding backfill", job: "knowledge_embedding_backfill", operationKey: "knowledge_embedding_backfill", eligibleKey: "embedding_backfill", defaultRecords: 25, maxRecords: 25, maxTotal: Number.MAX_SAFE_INTEGER, defaultBatches: 10, calibrationCap: 250, button: "Start embedding backfill", description: "Generate missing or stale embeddings across interactions, memories, intelligence, and Knowledge." },
    knowledge_generation: { label: "Knowledge generation", job: "run_all_knowledge_generation", operationKey: "run_all_knowledge_generation", eligibleKey: "knowledge_generation", defaultRecords: 100, maxRecords: 1000, maxTotal: Number.MAX_SAFE_INTEGER, defaultBatches: 1, calibrationCap: 250, button: "Generate Knowledge from source evidence", description: "Process eligible source evidence through every enabled Knowledge generation pathway." },
    hygiene_analysis: { label: "Knowledge hygiene analysis", job: "knowledge_hygiene_run", operationKey: "knowledge_hygiene_run", eligibleKey: "hygiene_analysis", defaultRecords: 250, maxRecords: 5000, maxTotal: Number.MAX_SAFE_INTEGER, defaultBatches: 1, calibrationCap: 250, button: "Analyze Knowledge for consolidation", description: "Discover candidates locally; proposal modes can use discounted asynchronous provider processing." },
    facet_backfill: { label: "Facet backfill", job: "backfill_facets", operationKey: "backfill_facets", eligibleKey: "facet_backfill", defaultRecords: 25, maxRecords: 100, maxTotal: Number.MAX_SAFE_INTEGER, defaultBatches: 10, calibrationCap: 250, button: "Backfill missing Knowledge facets", description: "Extract governed facets for active Knowledge records that currently have none." },
};
...
function KnowledgeOperations({ runs, controls, eligibleCounts, eligibleCountsLoading, eligibleCountsError, onRefreshEligible, onRefresh, settings }) {
    ...
    const resolvedBatches = runExtent === "all" ? (eligibleKnown ? Math.ceil(eligible / Math.max(1, recordsPerBatch)) : 0) : Math.max(1, batchesPerRun);
    const requestedRecords = recordsPerBatch * resolvedBatches;
    const totalRecords = eligibleKnown ? (runExtent === "all" ? eligible : Math.min(eligible, requestedRecords)) : 0;
    const calibrationCap = definition.calibrationCap || 250;
    const supportsInlineCalibration = operation === "embedding_backfill";
    const batchCapability = useMemo(
        () => batchCapabilities?.[definition.operationKey] || {},
        [batchCapabilities, definition.operationKey],
    );
    ...
    const selectedTarget = (batchCapability.targets || []).find(item => String(item.config_id) === String(providerConfigId)) || (batchCapability.targets || []).find(item => item.is_default);

    useEffect(() => {
        const target = (batchCapability.targets || []).find(item => item.is_default) || (batchCapability.targets || [])[0];
        if (!target) return;
        setProviderConfigId(String(target.config_id)); setBatchModel(target.model_name || "");
    }, [definition.operationKey, batchCapability]);
    ...
    const operationOptions = () => ({ records_per_batch: recordsPerBatch, batches_per_run: runExtent === "all" ? undefined : resolvedBatches, run_all: runExtent === "all", max_records: totalRecords, max_clusters: maxClusters, mode: operation === "hygiene_analysis" ? (settings?.knowledge_hygiene_mode || "analysis_only") : undefined, provider_config_id: providerConfigId || undefined, model_name: batchModel || undefined });
    ...
    const start = async () => {
        ...
        try {
            ...
            await Promise.all([onRefresh(), onRefreshEligible()]);
...
function ProviderBatchRunStatus({ runs, onRefresh }) {
    ...
    return <Card className="border-sky-500/40 bg-sky-500/5"><CardHeader className="pb-3"><CardTitle className="text-base flex items-center gap-2"><BarChart3 className="w-4 h-4" />Asynchronous provider batches</CardTitle><CardDescription>Each operation is a durable parent run. It prepares provider-safe child batches progressively, survives deployments, and never loads the full workload into memory.</CardDescription></CardHeader><CardContent className="space-y-3">{active.map(run => <div key={run.id} className="rounded-md border p-3 space-y-2"><div className="flex justify-between gap-2"><span className="text-xs font-medium">{JOB_LABELS[run.operation_key] || run.operation_key}</span><div className="flex gap-2"><Badge variant="outline">{run.local_status}</Badge><Badge variant="outline">{Number(run.child_count || 0)} provider job(s)</Badge></div></div><div className="grid grid-cols-2 md:grid-cols-5 gap-2 text-[11px] text-muted-foreground"><div>Sources prepared<br/><span className="text-foreground">{Number(run.prepared_source_count || 0).toLocaleString()} / {Number(run.target_source_count || 0).toLocaleString()}</span></div><div>Provider requests<br/><span className="text-foreground">{Number(run.request_count || 0).toLocaleString()}</span></div><div>Applied<br/><span className="text-foreground">{Number(run.applied_count || 0).toLocaleString()}</span></div><div>Failed<br/><span className="text-foreground">{Number(run.failed_count || 0).toLocaleString()}</span></div><div>Model<br/><span className="text-foreground">{run.provider} / {run.model}</span></div></div><div className="flex gap-2 flex-wrap"><Button size="sm" variant="outline" disabled={busy} onClick={() => command(run, "sync-status")}>Prepare / sync now</Button>{!run.pause_requested && <Button size="sm" variant="outline" disabled={busy} onClick={() => command(run, "pause")}>Pause new submissions</Button>}{run.pause_requested && !run.cancel_requested && <Button size="sm" variant="outline" disabled={busy} onClick={() => command(run, "resume")}>Resume</Button>}<Button size="sm" variant="outline" disabled={busy || run.cancel_requested} onClick={() => command(run, "cancel")}>Cancel operation</Button></div></div>)}{!active.length && <p className="text-xs text-muted-foreground">No asynchronous provider batches are currently active.</p>}</CardContent></Card>;
}
